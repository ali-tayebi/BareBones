...
{
    internal sealed class OutboxHostedService : IHostedService
    {
        ...
        private async Task SendOutboxMessagesAsync()
        {
            ...
            {
                var outbox = scope.ServiceProvider.GetRequiredService<IMessageOutboxRepository>();
                ...
                {
                    // await _publisher.PublishAsync(message.Message, message.IdentityKey, message.CorrelationId,
                    //     message.SpanContext, message.MessageContext, message.Headers);

                    await _publisher.PublishAsync(message.IdentityKey, message.Message, message.Headers);
